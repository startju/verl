...
if [ $USE_SGLANG -eq 1 ]; then
    pip install "sglang[all]==0.5.2" -v && pip install torch-memory-saver -v
fi
...
if [ $USE_MEGATRON -eq 1 ]; then
    ...
    NVTE_FRAMEWORK=pytorch pip3 install -v --no-deps git+https://github.com/NVIDIA/TransformerEngine.git@v2.6
    pip3 install -v --no-deps git+https://github.com/NVIDIA/Megatron-LM.git@core_v0.13.1
fi


echo "5. May need to fix opencv"
pip install -v opencv-python
pip install -v opencv-fixer && \
    python -c "from opencv_fixer import AutoFix; AutoFix()"


if [ $USE_MEGATRON -eq 1 ]; then
    echo "6. Install cudnn python package (avoid being overridden)"
    pip install -v nvidia-cudnn-cu12==9.10.2.21
fi
